...
def test_get_lines(file_contents):
    ...
    Mock open_file() results since tested elsewhere.
    """
    file_path = "foo/bar"
...
class TestCSVParser:
    ...
    def test_get_records(
        ...
        Test relies on helper methods tested in TestTSVParser.
        """
        mock_get_lines.return_value = lines
...
class TestGenBankParser:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ([], {}),
            (GENBANK_COMMENT_WORDS, {parsers.GenBankParser.COMMENT_FIELD:
                GENBANK_COMMENT_LINE}),
            (GENBANK_SUMMARY_WORDS, {parsers.GenBankParser.SUMMARY_FIELD:
                GENBANK_SUMMARY_CONTENTS}),
...
class TestXMLParser:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("foo[@id=buz]/bar[@name=fu][@id=bur]", [("foo", {"id": "buz"}), ("bar", {"name": "fu", "id":
                "bur"})]),
        ]
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("foo[@name=fu]/bar", [Element("foo", attrib={"name": "fu"}),
                Element("bar")], True),
            ("foo[@name=fu]/bar", [Element("foo", attrib={"name": "bur"}),
                Element("bar")], False),
        ]
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (Element("foo", text="bar"), {"fu": "bur"}, None, None, {"fu": "bur", "foo": "bar"}),
            (Element("foo", text="empty"), {}, None, None, {}),
            (Element("foo", text="item1;item2"), {}, None, None, {"foo": ["item1", "item2"]}),
            (Element("foo", text="bar"), {"foo": "bur"}, None, None, {"foo": ["bur", "bar"]}),
            (Element("foo", text="bar"), {}, {("fu", 2)}, 1, {}),
            (Element("foo", text="bar"), {"fu": "bur"}, {("fu", 2)}, 1, {"foo": {"fu": "bur"}}),
        ]
    )
    def test_add_element_to_record(self, element, record, children, depth, expected):
        ...
        list_identifier = ";"
        empty_fields = ["empty"]
        parser = parsers.XMLParser(None, list_identifier=list_identifier,
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("foo", []),
            ("people/body/tissues/tissue", [{"cell": ["RBC", "WBC"]}, {"cell": ["Neuron",
                "Astrocyte"]}]),
            ("people/body/tissues/tissue[@type=blood]", [{"cell": ["RBC", "WBC"]}]),
            ("people/body/tissues/tissue[@type=blood]/cell", [{"cell": "RBC"}, {"cell":
                "WBC"}]),
            ("people/body/limbs/arm", [{"arm": "Left arm"}, {"arm": "Right arm"}]),
            ("people/body/limbs", [{"arm": ["Left arm", "Right arm"], "leg": ["Left leg", "Right leg"]}]),
        ]
    )
    def test_get_records(self, record_path, expected):
        """"""
        with mock.patch("cgap_gene_annotation.src.utils.open",
