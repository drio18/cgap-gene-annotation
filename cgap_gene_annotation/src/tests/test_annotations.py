...
from ..annotations import SourceAnnotation, GeneAnnotation, JSONInputError
...
ANNOTATION_METADATA_1 = {
    ...
    constants.PARSER: {constants.PARSER_CHOICE:
        list(constants.PARSERS_AVAILABLE.keys())[0]},
...
TEST_ANNOTATION_FILE_CONTENTS = {
    constants.METADATA: TEST_METADATA,
    constants.ANNOTATION: TEST_ANNOTATIONS,
}
ANNOTATIONS_TO_ADD = [
    {"foo": "bar"},
    {"fu": "bur"},
...
def record():
    return deepcopy(TEST_ANNOTATION_RECORD)
...
def basic_gene_annotation():
    """"""
    gene_annotation = GeneAnnotation(TEST_FILE_PATH)
    gene_annotation.metadata = deepcopy(TEST_METADATA)
    gene_annotation.annotations = deepcopy(TEST_ANNOTATIONS)
...
class TestGeneAnnotation:
    ...
    def test_validate_json(self, json_input, raise_error, empty_gene_annotation):
        """"""
        schema = {"type": "object", "additionalProperties": False, "properties": {"foo": {"type": "string"}}}
    ...
    @pytest.mark.parametrize(
        ...
        [
            ({}, {}, []),
            ({constants.METADATA: TEST_METADATA}, TEST_METADATA, []),
            ({constants.ANNOTATION: TEST_ANNOTATIONS}, {}, TEST_ANNOTATIONS),
            ({constants.METADATA: TEST_METADATA, constants.ANNOTATION:
                TEST_ANNOTATIONS}, TEST_METADATA, TEST_ANNOTATIONS),
        ]
    )
    def test_parse_file(self, file_contents, expected_metadata,
            expected_annotations, empty_gene_annotation):
        """"""
        file_handle = "foo bar"
        open_file_value = (n for n in [file_handle])
        with mock.patch("cgap_gene_annotation.src.annotations.open_file",
                return_value=open_file_value) as mock_open_file:
            with mock.patch(
    ...
    def test_create_parser(self, empty_gene_annotation):
        ...
        for key, value in constants.PARSERS_AVAILABLE.items():
            ...
            parser = empty_gene_annotation.create_parser(TEST_FILE_PATH, parser_metadata)
    ...
    def test_write_file(self, basic_gene_annotation):
        ...
        with tempfile.NamedTemporaryFile() as tmp:
            ...
            assert json.load(tmp) == TEST_ANNOTATION_FILE_CONTENTS
    ...
    def test_add_source(self, annotation_metadata,
            annotations_to_add, empty_gene_annotation):
        """"""
        prefix = annotation_metadata.get(constants.PREFIX)
        base_annotation = annotation_metadata.get(constants.SOURCE)
        with mock.patch("cgap_gene_annotation.src.annotations.SourceAnnotation",
                new=self.mocked_source_annotation(annotations_to_add)) as mock_source_annotation:
            with mock.patch("cgap_gene_annotation.src.annotations.AnnotationMerge") as mock_merge:
                ...
                if base_annotation is True:
                    expected = [{prefix: [annotation]} for annotation in
                        annotations_to_add]
    ...
    def test_replace_annotations(self, mock_remove_identifier, mock_add_source,
            basic_gene_annotation):
        """"""
    ...
    def test_create_annotation(self, mock_add_annotations, json_input, raise_error, empty_gene_annotation):
        """"""
        if not raise_error:
    ...
    @mock.patch("cgap_gene_annotation.src.annotations.GeneAnnotation.remove_annotations")
    @mock.patch("cgap_gene_annotation.src.annotations.GeneAnnotation.replace_annotations")
    @mock.patch("cgap_gene_annotation.src.annotations.GeneAnnotation.add_annotations")
    @pytest.mark.parametrize(
        "json_input,raise_error",
        [
    ...
    def test_update_annotation(self, mock_add_annotations, mock_replace_annotations,
            mock_remove_annotations, mock_parse_file, json_input, raise_error, empty_gene_annotation):
        """"""
        if not raise_error:
